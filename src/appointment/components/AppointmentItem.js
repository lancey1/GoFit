...
function AppointmentItem(props) {
    ...
    const { onRefresh, id, title, description, revieverAvatar, reviewRecieverId, address, recieverName, appointmentDate, location, pending, recieverAccepted, recieverRejected, creatorName, creatorAvatar, reviews, all } = props;
        ...
        <React.Fragment>
            ...
            {showReview && (
                <BackDrop onClear={hideShowReview}>
                    <NewReview appointmentId={id} reviewRecieverId={reviewRecieverId} all onRefresh={onRefresh} />
                </BackDrop>
            )}

            <article onClick={btnClickHandler} className={`${styles.article}`} >
                ...
                <div className={`${styles.user}`} >
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" className={`${styles.arrow} h-6 w-6`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                ...
                {(appointmentFinished && recieverAccepted && !displayReviewBtn) && (
                    <div className={`${styles.rating_div}`}>
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="#ffcd3c">
